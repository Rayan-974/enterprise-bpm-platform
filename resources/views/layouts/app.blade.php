...
<body class="bg-creamBase min-h-screen text-slate-800 flex flex-col antialiased">
    ...
    <!-- Top Navigation Bar -->
    <header class="bg-purpleSecondary text-white shadow-xl sticky top-0 z-50 border-b-2 border-skyPrimary/40 backdrop-blur-md">
        <div class="max-w-[1600px] w-full mx-auto px-4 sm:px-8 lg:px-12 py-3.5 sm:py-4 flex items-center justify-between">
            <div class="flex items-center space-x-4 sm:space-x-6">
                <button id="mobile-menu-btn" aria-label="Toggle Navigation Menu" class="md:hidden text-white hover:text-skyPrimary focus:outline-none p-2 rounded-xl hover:bg-purpleHover transition min-h-[44px] min-w-[44px] flex items-center justify-center">
    ...
    <!-- Main Content Layout -->
    <div class="flex-1 max-w-[1600px] w-full mx-auto px-4 sm:px-8 lg:px-12 py-6 sm:py-8 flex flex-col md:flex-row gap-6 lg:gap-10">
        <!-- Sidebar Navigation -->
        <aside id="sidebar" class="w-full md:w-64 lg:w-72 flex-shrink-0 bg-white/95 backdrop-blur-md rounded-3xl shadow-xl border border-slate-200/80 p-5 hidden md:block self-start sticky top-24 z-40 transition-all">
        ...
        <!-- Main Content Area -->
        <main class="flex-1 w-full min-w-0 page-fade-up">
            @if(session('success'))
                <div class="mb-5 p-4 rounded-2xl bg-emerald-50/90 border border-emerald-300 text-emerald-900 text-sm font-semibold flex items-center justify-between shadow-lg">
                    <span>{{ session('success') }}</span>
                </div>
            @endif

            @if(session('error'))
                <div class="mb-5 p-4 rounded-2xl bg-rose-50/90 border border-rose-300 text-rose-900 text-sm font-semibold flex items-center justify-between shadow-lg">
